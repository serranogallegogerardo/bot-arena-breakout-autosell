...
def calculate_brightness(image):
    """
    Calcula el brillo promedio de una imagen (o una región de la imagen) en escala de grises.
    """
...
def find_item_position(item_image):
    ...
    if max_val >= threshold:
        ...
        # Extraer la región del ítem encontrado
        item_region = screenshot[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
        
        # Calcular el brillo de la región del ítem
        brightness = calculate_brightness(item_region)
        print(f"Brillo del ítem: {brightness}")
        
        # Umbral para decidir si es vendible (ajusta este valor según tus necesidades)
        brillo_umbral = 50
        
        if brightness < brillo_umbral:
            print("El ítem es demasiado oscuro, no es vendible.")
            return None
